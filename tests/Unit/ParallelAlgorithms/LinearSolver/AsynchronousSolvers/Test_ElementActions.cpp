...
SPECTRE_TEST_CASE("Unit.ParallelLinearSolver.Asynchronous.ElementActions",
...

  {
    ...
    CHECK(get_tag(Parallel::Tags::IterationId<TestSolver>{}) ==
          std::numeric_limits<size_t>::max());
    tmpl::for_each<tmpl::list<operator_applied_to_fields_tag, residual_tag,
                              Parallel::Tags::HasConverged<TestSolver>>>(
  ...
  }
  {
    ...
    ActionTesting::next_action<element_array>(make_not_null(&runner),
                                              element_id);
    CHECK(get_tag(Parallel::Tags::IterationId<TestSolver>{}) == 0);
    CHECK_FALSE(get_tag(Parallel::Tags::HasConverged<TestSolver>{}));
    ActionTesting::invoke_queued_simple_action<obs_component>(
        make_not_null(&runner), 0);
  ...
  }
  {
    ...
    ActionTesting::next_action<element_array>(make_not_null(&runner),
                                              element_id);
    CHECK(get_tag(Parallel::Tags::IterationId<TestSolver>{}) == 1);
    CHECK(get_tag(Parallel::Tags::HasConverged<TestSolver>{}));
    ActionTesting::invoke_queued_simple_action<obs_component>(
        make_not_null(&runner), 0);
    ActionTesting::invoke_queued_threaded_action<obs_writer>(
        make_not_null(&runner), 0);
