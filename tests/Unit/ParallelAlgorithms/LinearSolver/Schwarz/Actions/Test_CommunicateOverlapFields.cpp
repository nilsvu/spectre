...
void test_communicate_overlap_fields(const size_t num_points_per_dim,
  ...
  // Setup element array
  const auto add_element = [num_points_per_dim, overlap,
    ...
    for (size_t i = 0; i < mesh.number_of_grid_points(); ++i) {
      get(get<ScalarFieldTag<0>>(fields))[i] = i;
      get(get<ScalarFieldTag<1>>(fields))[i] = i;
    }
...
SPECTRE_TEST_CASE("Unit.ParallelSchwarz.Action.CommunicateOverlapFields",
  ...
  test_communicate_overlap_fields<1, false>(3, 2, {0., 1., 2.});
  test_communicate_overlap_fields<1, true>(3, 2, {0., 1.});
  test_communicate_overlap_fields<2, false>(
      3, 2, {0., 1., 2., 3., 4., 5., 6., 7., 8.});
  test_communicate_overlap_fields<2, true>(3, 2, {0., 1., 3., 4., 6., 7.});
  test_communicate_overlap_fields<3, false>(2, 1,
                                            {0., 1., 2., 3., 4., 5., 6., 7.});
  test_communicate_overlap_fields<3, true>(2, 1, {0., 2., 4., 6.});
}
