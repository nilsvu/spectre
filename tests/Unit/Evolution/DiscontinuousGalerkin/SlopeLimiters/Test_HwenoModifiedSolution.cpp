...
void test_hweno_modified_solution_1d() {
  INFO("Testing compute_hweno_modified_neighbor_solution in 1D");
  ...
  const auto upper_xi_vars = [&mesh]() noexcept {
    // Large values to make sure this neighbor is skipped
  ...
  SlopeLimiters::compute_hweno_modified_neighbor_solution<ScalarTag>(
  ...
  // found using Mathematica, using the following code:
  // qw3 = {1/3, 4/3, 1/3};
  // qx3 = {-1, 0, 1};
  ...
  const auto expected = [&logical_coords]() noexcept {
    const auto& x = get<0>(logical_coords);
    const double c0 = 41. / 30.;
    const double c1 = -31. / 10.;
    const double c2 = -7. / 10.;
    return Scalar<DataVector>{{{c0 + c1 * x + c2 * square(x)}}};
  }
  ();
...
// check that different components will correctly skip different neighbors if
// needed.
// Multiple components becomes very tedious in 3D, so 3D will test a scalar.
void test_hweno_modified_solution_2d_vector() {
  INFO("Testing compute_hweno_modified_neighbor_solution in 2D");
  ...
  const auto lower_xi_vars = [&mesh]() noexcept {
    Variables<TagsList> result(mesh.number_of_grid_points());
    // The x component should skip this neighbor
  ...
  const auto upper_eta_vars = [&mesh, &logical_coords ]() noexcept {
    ...
    // The y component should skip this neighbor
  ...
  SlopeLimiters::compute_hweno_modified_neighbor_solution<VectorTag<2>>(
      make_not_null(&modified_tensor), local_tensor, element, mesh,
      neighbor_data, primary_neighbor);

  // The expected coefficient values for the result of the constrained fit are
  // found using Mathematica, using the following code. This example computes
  // the expected result for the vector x component; an analogous piece of code
  // gives the expected result for the y component, but note that the neighbor
  // to skip changes in this case.
  // qw3 = {1/3, 4/3, 1/3};
  // qx3 = {-1, 0, 1};
  // qw4 = {1/6, 5/6, 5/6, 1/6};
  ...
  const auto expected = [&logical_coords]() noexcept {
    ...
    const double c0 = 6049013. / 5913219.;
    const double c1 = 92367. / 360620.;
    const double c2 = -1659. / 558961.;
    const double c3 = -6083. / 558961.;
    const double c4 = 61831556. / 187251935.;
    const double c5 = 42. / 6935.;
    const double c6 = -126. / 42997.;
    const double c7 = -462. / 42997.;
    const double c8 = -2460491. / 37450387.;
    const double c9 = 18283. / 180310.;
    const double c10 = -378. / 558961.;
    const double c11 = -1386. / 558961.;
    // y-component coefficients
    const double d0 = 991516363. / 468183825.;
    const double d1 = 1142461. / 1991042.;
    const double d2 = -686461. / 1224010.;
    const double d3 = -130350. / 995521.;
    const double d4 = 110206607. / 156061275.;
    const double d5 = 72540. / 995521.;
    const double d6 = -55692. / 122401.;
    const double d7 = -128700. / 995521.;
    const double d8 = 10878374. / 52020425.;
    const double d9 = 16740. / 995521.;
    const double d10 = -6119. / 1224010.;
    const double d11 = -29700. / 995521.;
    return tnsr::I<DataVector, 2>{
        {{DataVector{c0 + c1 * x + c2 * square(x) + c3 * cube(x) +
                     y * (c4 + c5 * x + c6 * square(x) + c7 * cube(x)) +
                     square(y) *
                         (c8 + c9 * x + c10 * square(x) + c11 * cube(x))},
          DataVector{d0 + d1 * x + d2 * square(x) + d3 * cube(x) +
                     y * (d4 + d5 * x + d6 * square(x) + d7 * cube(x)) +
                     square(y) *
                         (d8 + d9 * x + d10 * square(x) + d11 * cube(x))}}}};
  }
  ();
...
void test_hweno_modified_solution_3d() {
  INFO("Testing compute_hweno_modified_neighbor_solution in 3D");
  using TagsList = tmpl::list<ScalarTag>;
  struct PackagedData {
    tuples::TaggedTuple<::Tags::Mean<ScalarTag>> means;
    Variables<TagsList> volume_data;
  ...
  const auto expected = [&logical_coords]() noexcept {
    ...
    const auto& y = get<1>(logical_coords);
    const auto& z = get<2>(logical_coords);
    const double c0 = 7734190419582420551. / 62148673763526035437.;
    const double c1 = 765121. / 1263364.;
    const double c2 = 786240. / 1895041.;
    const double c3 = 124800. / 1105441.;
    const double c4 = 0.;
    const double c5 = 0.;
    const double c6 = 374400. / 1105441.;
    const double c7 = 0.;
    const double c8 = 0.;
    const double c9 = 308447321809657439079. / 621486737635260354370.;
    const double c10 = -892944. / 1579205.;
    const double c11 = -6250608. / 9475205.;
    const double c12 = -878879. / 11054410.;
    const double c13 = 0.;
    const double c14 = 0.;
    const double c15 = -595296. / 1105441.;
    const double c16 = 1. / 2.;
    const double c17 = 0.;
    const double c18 = 21514562148445593021. / 124297347527052070874.;
    const double c19 = 89424. / 315841.;
    const double c20 = 625968. / 1895041.;
    const double c21 = 99360. / 1105441.;
    const double c22 = 0.;
    const double c23 = 0.;
    const double c24 = 298080. / 1105441.;
    const double c25 = 0.;
    const double c26 = 0.;
    const double c27 = 3655223967127444271. / 310743368817630177185.;
    const double c28 = -14256. / 315841.;
    const double c29 = -99792. / 1895041.;
    const double c30 = -15840. / 1105441.;
    const double c31 = 0.;
    const double c32 = 0.;
    const double c33 = -47520. / 1105441.;
    const double c34 = 0.;
    const double c35 = 0.;
    const DataVector term_z0 = c0 + c1 * x + c2 * square(x) +
                               y * (c3 + c4 * x + c5 * square(x)) +
                               square(y) * (c6 + c7 * x + c8 * square(x));
    const DataVector term_z1 = c9 + c10 * x + c11 * square(x) +
                               y * (c12 + c13 * x + c14 * square(x)) +
                               square(y) * (c15 + c16 * x + c17 * square(x));
    const DataVector term_z2 = c18 + c19 * x + c20 * square(x) +
                               y * (c21 + c22 * x + c23 * square(x)) +
                               square(y) * (c24 + c25 * x + c26 * square(x));
    const DataVector term_z3 = c27 + c28 * x + c29 * square(x) +
                               y * (c30 + c31 * x + c32 * square(x)) +
                               square(y) * (c33 + c34 * x + c35 * square(x));
...
void test_hweno_modified_solution_2d_boundary() {
  INFO("Testing compute_hweno_modified_neighbor_solution in 2D at boundary");
  using TagsList = tmpl::list<ScalarTag>;
  ...
  const auto expected = [&logical_coords]() noexcept {
    const auto& x = get<0>(logical_coords);
    const auto& y = get<1>(logical_coords);
    const double c0 = 50738. / 49647.;
    const double c1 = 1. / 4.;
    const double c2 = 0.;
    const double c3 = 0.;
    const double c4 = 27242. / 82745.;
    const double c5 = 0.;
    const double c6 = 0.;
    const double c7 = 0.;
    const double c8 = -1091. / 16549.;
    const double c9 = 1. / 10.;
    const double c10 = 0.;
    const double c11 = 0.;
    return Scalar<DataVector>{
        {{c0 + c1 * x + c2 * square(x) + c3 * cube(x) +
          y * (c4 + c5 * x + c6 * square(x) + c7 * cube(x)) +
          square(y) * (c8 + c9 * x + c10 * square(x) + c11 * cube(x))}}};
  }
  ();
...
void test_hweno_modified_solution_3d_boundary() {
  INFO("Testing compute_hweno_modified_neighbor_solution in 3D at boundary");
  using TagsList = tmpl::list<ScalarTag>;
  struct PackagedData {
    tuples::TaggedTuple<::Tags::Mean<ScalarTag>> means;
    Variables<TagsList> volume_data;
  ...
  const auto expected = [&logical_coords]() noexcept {
    ...
    const auto& y = get<1>(logical_coords);
    const auto& z = get<2>(logical_coords);
    const double c0 = 243751581645799. / 689207629948649.;
    const double c1 = 765121. / 1263364.;
    const double c2 = 786240. / 1895041.;
    const double c3 = 124800. / 1105441.;
    const double c4 = 0.;
    ...
    const double c7 = 0.;
    const double c8 = 0.;
    const double c9 = 1416550233603063. / 1378415259897298.;
    const double c10 = -892944. / 1579205.;
    const double c11 = -6250608. / 9475205.;
    const double c12 = -878879. / 11054410.;
    const double c13 = 0.;
    ...
    const double c16 = 1. / 2.;
    const double c17 = 0.;
    const double c18 = -709303092297615. / 1378415259897298.;
    const double c19 = 89424. / 315841.;
    const double c20 = 625968. / 1895041.;
    const double c21 = 99360. / 1105441.;
    const double c22 = 0.;
    ...
    const double c25 = 0.;
    const double c26 = 0.;
    const double c27 = 627297076397287. / 3446038149743245.;
    const double c28 = -14256. / 315841.;
    const double c29 = -99792. / 1895041.;
    const double c30 = -15840. / 1105441.;
    const double c31 = 0.;
