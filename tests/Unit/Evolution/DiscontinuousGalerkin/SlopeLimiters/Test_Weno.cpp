...
SPECTRE_TEST_CASE("Unit.Evolution.DG.SlopeLimiters.Weno.Serialization",
                  "[SlopeLimiters][Unit]") {
  const SlopeLimiters::Weno<1, tmpl::list<scalar, vector<1>>> sweno(
      SlopeLimiters::WenoType::SimpleWeno, 0.001);
  test_serialization(sweno);

  const SlopeLimiters::Weno<1, tmpl::list<scalar, vector<1>>> sweno2(
      SlopeLimiters::WenoType::SimpleWeno, 0.001, true);
  CHECK(sweno != sweno2);
}
...
void test_simple_weno_1d(const double rhs_constant,
  ...
  using SimpleWenoLimiter = SlopeLimiters::Weno<1, tmpl::list<scalar>>;
  ...
  const auto lower_xi_vars = make_lower_xi_vars(logical_coords, 2.0);
  ...
  neighbor_data[lower_xi].mesh = mesh;
  neighbor_data[lower_xi].element_size = element_size;
  neighbor_data[upper_xi].volume_data = upper_xi_vars;
  neighbor_data[upper_xi].mesh = mesh;
  neighbor_data[upper_xi].element_size = element_size;

  const SimpleWenoLimiter sweno(SlopeLimiters::WenoType::SimpleWeno, 0.001);
  auto s = get<scalar>(vars);

  const bool activated =
      sweno(make_not_null(&s), element, mesh, element_size, neighbor_data);

  if (expected_activation) {
    CHECK(activated);

    // expect smoothness indicators:
    // center_beta = integral (-2 + 2x)^2 dx + integral (2)^2 dx
    // lower_xi_beta = integral (-10 -2x)^2 dx + integral (-2)^2 dx
    // upper_xi_beta = integral (-1 + x)^2 dx + integral (1)^2 dx
    const double center_beta = 56.0 / 3.0;
    const double lower_xi_beta = 632.0 / 3.0;
    const double upper_xi_beta = 14.0 / 3.0;
...
void test_simple_weno_2d(const double xi_rhs_constant,
                         ...
                         const bool expected_activation) noexcept {
  using SimpleWenoLimiter = SlopeLimiters::Weno<2, tmpl::list<scalar>>;
  ...
  neighbor_data[lower_eta].element_size = element_size;
  neighbor_data[upper_eta].volume_data = upper_eta_vars;
  neighbor_data[upper_eta].mesh = mesh;
  neighbor_data[upper_eta].element_size = element_size;
  ...
  if (expected_activation) {
    ...
    const double upper_xi_beta = 92.0 / 3.0;
    const double lower_eta_beta = 5.0;
    const double upper_eta_beta = 92.0 / 3.0;
    // unnormalized weights
    const double gamma = 0.001;
...
SPECTRE_TEST_CASE(
    "Unit.Evolution.DG.SlopeLimiters.Weno.1d_2d_detailed_action_test",
    "[SlopeLimiters][Unit]") {
