...
void test_massive_restriction() {
  ...
  // `::dg::apply_mass` is exact. Note that for Gauss-Lobatto quadrature the
  // mass matrix is diagonally approximated in most places in the code, but the
  // `projection_matrix_child_to_parent` uses the exact mass matrix because it
  // is implemented in terms of Vandermonde matrices.
  ...
  for (const ChildSize child_size :
    ...
    ::dg::apply_mass(make_not_null(&lhs), parent_mesh);
    // (ii) Compute r.h.s.
    auto massive_child_data = child_data;
    ::dg::apply_mass(make_not_null(&massive_child_data), child_mesh);
...
void test_exact_restriction() {
  ...
  ::dg::apply_mass(make_not_null(&parent_data), parent_mesh);
  ...
  // above). The `apply_mass` function requires pre-multiplying by the Jacobian.
  child_data_left *= 2.;
  child_data_right *= 2.;
  ::dg::apply_mass(make_not_null(&child_data_left), child_mesh);
  ::dg::apply_mass(make_not_null(&child_data_right), child_mesh);
