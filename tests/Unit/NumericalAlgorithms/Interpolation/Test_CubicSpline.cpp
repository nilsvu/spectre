...
void test_cubic_spline(const F& function, const double lower_bound,
  ...
  for (size_t i = 0; i < 10 * size; ++i) {
    ...
    max_error = std::max(abs(interpolated_y_value - y_value), max_error);
    ...
    if (i > 10 * size * boundary_fraction and
      ...
      max_error_interior =
          std::max(abs(interpolated_y_value - y_value), max_error_interior);
  ...
  CHECK(custom_approx(max_error) == 0.);
  CHECK(custom_approx_interior(max_error_interior) == 0.);
...
SPECTRE_TEST_CASE("Unit.Numerical.Interpolation.CubicSpline",
  ...
  test_with_polynomial(100, 2, 1.e-3, 1.e-14);
  test_with_polynomial(100, 3, 1.e-2, 1.e-14);
  test_with_polynomial(1000, 3, 1.e-3, 1.e-14);
  test_with_polynomial(1000, 4, 1.e-3, 1.e-8);
  test_with_polynomial(1000, 5, 1.e-3, 1.e-7);
}
