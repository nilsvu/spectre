...
template <class FluxType, class... Args,
          typename PackagedData =
              dg::SimpleBoundaryData<typename FluxType::package_field_tags,
                                     typename FluxType::package_extra_tags>>
PackagedData get_packaged_data(const FluxType& flux_computer,
                               const DataVector& used_for_size,
                               const Args&... args) noexcept {
  PackagedData packaged_data{used_for_size.size()};
...
void test_conservation(const FluxType& flux_computer,
  ...
  CHECK_VARIABLES_APPROX(n_dot_num_flux_interior,
                         -1. * n_dot_num_flux_exterior);
}
}  // namespace detail
