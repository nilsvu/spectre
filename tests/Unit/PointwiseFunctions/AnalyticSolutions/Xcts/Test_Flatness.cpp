...
SPECTRE_TEST_CASE("Unit.PointwiseFunctions.AnalyticSolutions.Xcts.Flatness",
  ...
  CHECK(get<0>(get<Tags::ShiftExcess<DataVector, 3, Frame::Inertial>>(vars)) ==
        DataVector(num_points, 0.));
  CHECK(get<0, 0>(get<Tags::ConformalMetric<DataVector, 3, Frame::Inertial>>(
            vars)) == DataVector(num_points, 1.));
  CHECK(get<0, 1>(get<Tags::ConformalMetric<DataVector, 3, Frame::Inertial>>(
            vars)) == DataVector(num_points, 0.));
}
