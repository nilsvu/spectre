...
/// \file Helper functions to test elliptic first-order systems
...
void test_first_order_fluxes_computer(
  ...
  const auto computed_fluxes = tuples::apply(
      [&vars, &fluxes_computer](const auto&... args) {
        return ::elliptic::first_order_fluxes<volume_dim, primal_fields,
                                              auxiliary_fields>(
            vars, fluxes_computer, args...);
      },
      args);
  ...
  auto box = tuples::apply(
      [&vars, &used_for_size](const auto&... args) {
        return db::create<
            ...
            vars,
            make_with_value<db::item_type<fluxes_tag>>(
...
void test_first_order_sources_computer(const DataVector& used_for_size) {
  ...
  using fluxes_tag =
      db::add_tag_prefix<::Tags::Flux, vars_tag, tmpl::size_t<volume_dim>,
                         Frame::Inertial>;
  using FluxesType = db::item_type<fluxes_tag>;
  ...
  auto box = tuples::apply(
      [&vars, &fluxes, &used_for_size](const auto&... args) {
  ...
  tuples::apply(
      [&box](const auto&... vars) {
        db::mutate_apply<
            ...
            argument_tags>(SourcesComputer{}, make_not_null(&box), vars...);
      },
      vars_tuple);
