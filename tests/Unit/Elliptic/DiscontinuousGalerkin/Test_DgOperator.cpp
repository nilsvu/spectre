      ...
      Parallel::PhaseActions<
          ...
          tmpl::list<
              ...
              ::elliptic::dg::Actions::apply_operator<
                  System, true, TemporalIdTag, vars_tag,
                  operator_applied_to_vars_tag, primal_fluxes_tag>,
...
auto test_dg_operator(
  ...
  ActionTesting::MockRuntimeSystem<Metavars> runner{
      tuples::TaggedTuple<domain::Tags::Domain<Dim>,
          ...
          std::move(domain), penalty_parameter, false, std::move(analytic_solution_ptr)}};
  const auto& analytic_solution =
      get<::Tags::AnalyticSolution<AnalyticSolution>>(runner.cache());
          ...
          for (const auto& [element_id, expected_operator_applied_to_vars] :
            ...
            typename local_operator_applied_to_vars_tag::type
                discretization_error_pointwise =
                    operator_applied_to_vars - expected_operator_applied_to_vars;
            const auto& local_element_id = element_id;
            tmpl::for_each<typename local_operator_applied_to_vars_tag::tags_list>(
                [&local_element_id, &discretization_error_pointwise,
                 &discretization_errors](auto tag_v) {
                  using tag = tmpl::type_from<std::decay_t<decltype(tag_v)>>;
                  get<ErrorTag<tag>>(discretization_errors[local_element_id]) =
  ...
  if constexpr (Linearized) {
    ...
    std::unordered_map<ElementId<Dim>, PrimalFluxesVars> all_zero_primal_fluxes{};
  ...
  }
  {
    ...
    std::unordered_map<ElementId<Dim>, PrimalFluxesVars> analytic_primal_fluxes{};
    std::unordered_map<ElementId<Dim>, OperatorAppliedToVars>
        analytic_fixed_source_with_inhom_bc{};
    for (const auto& element_id : all_element_ids) {
      const auto& inertial_coords = get_tag(
          domain::Tags::Coordinates<Dim, Frame::Inertial>{}, element_id);
      analytic_primal_vars[element_id] = variables_from_tagged_tuple(analytic_solution.variables(
          inertial_coords, typename System::primal_fields{}));
      analytic_primal_fluxes[element_id] = variables_from_tagged_tuple(analytic_solution.variables(
...
//         nullptr};
    ...
    // using PrimalFluxes = Variables<tmpl::list<Var<::Tags::deriv<
...
//                                    analytic_solution, analytic_solution_approx,
//                                    {});
//   }
//   {
//     INFO("2D rectilinear");
...
//   }

  {
    ...
    using system =
        Xcts::FirstOrderSystem<Xcts::Equations::HamiltonianLapseAndShift, Xcts::Geometry::Curved>;
