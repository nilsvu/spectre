Re-scoping note from the feature-parity survey (2026-08-13): the plan of record for initial orbital parameters is now the [SimulationSupport](https://github.com/sxs-collaboration/SimulationSupport) package (#7412), which carries a byte-compatible copy of the current SpEC PN kernels — its test asserts the same numbers SpECTRE asserts today, so adoption is a pure refactor.
