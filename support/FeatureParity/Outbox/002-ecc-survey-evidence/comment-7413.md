...
**G5 — no shared pre-branch history — being addressed by PR #7496.** Each branched Lev gets its own `PipelineStep` subdirectory (`support/Python/DirectoryStructure.py:132-193`); SpEC symlinks the shared parent segments into every Lev. PR #7496 ("Record the chain of runs that make up a simulation", open) adds the missing back-reference: a `ContinuedFrom:` field in the branch and ringdown input files that post-processing follows to collect everything belonging to one simulation. With that chain in place, the remaining delta vs SpEC is only the *shared on-disk history* (symlinks avoid duplicating pre-branch data) — worth keeping in mind for archiving, but no longer a correctness or post-processing gap.
...
4. **Add a Lev later.** CLI verb `spectre bbh add-lev -d <pipeline_dir> --lev N`: locates the branch-point run by following the `ContinuedFrom:` chain that PR #7496 introduces, calls `start_inspiral` with the recorded arguments, and refuses Levs already branched (discoverable from the same chain) — i.e. idempotent like SpEC's symlink check. Check PR #6717 (generic branch-runs command) as the home before adding a new verb; #7496 should merge first either way.
