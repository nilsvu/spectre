...
Consumers of the composed `PYTHONPATH`, all of which follow the discovered path: `bin/LoadPython.sh`, the Python test environment used by `spectre_add_python_test` and `spectre_add_python_bindings_test`, the input-file test environment (`AddInputFileTests.cmake`), the Catch test environment (`SpectreAddCatchTests.cmake`), and the configure-time module probes (`FindPythonModule.cmake`, `FindNumPy.cmake`). `SetupPybind11.cmake` used the assumed site-packages path as a `find_package` hint and now uses the discovered one — pybind11 is in `requirements.txt`, so it could be bootstrapped. `SetupSpec.cmake` still prepends SpEC's Python directories under `SPEC_ROOT`. Nothing else referenced the old path: the two `site-packages` mentions in `.github/workflows/Tests.yaml` are ParaView's and the CI interpreter's own, and the install rule in `SpectreSetupPythonPackage.cmake` targets `CMAKE_INSTALL_PREFIX`.
