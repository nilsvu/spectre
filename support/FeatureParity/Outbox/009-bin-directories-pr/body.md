...
It is a frozen dataclass in the style of `Segment`, `PipelineStep` and `Checkpoint` in `DirectoryStructure.py`: one `path` field, the rest of the layout derived from it (`python_dir`, `support_dir`), classmethod constructors (`running()`, `find()`, `create()`) and two operations (`add()`, `executable()`). `schedule` threads one handle instead of parallel paths, and converts to a plain path at the boundary where `SchedulerContext.yaml` is written.
...
**Next to it, `support/`** holds the support files: the submit script templates and the configured `Machine.yaml`. CMake configures them into `<build_dir>/support/` too (`support/CMakeLists.txt`), and `BinDirectory.create` copies them into the simulation, so **the support files are at the same place relative to `bin/` in both**. Both are found by one expression relative to the running CLI — `BinDirectory.running().support_dir` — which resolves to the build directory's copy in a build directory and to the simulation's in a simulation, with nothing to translate between them. The machine environment script of #7443 joins them there.
