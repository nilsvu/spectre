...
def sum_Fr4(r0, a, m_max, M=1.0):
    return (
        ...
        * r0**9
        * (a * M + r0 * np.sqrt(M * r0))
        * (2 * a * np.sqrt(M * r0) + r0 * (-3 * M + r0)) ** 3.5
        * (a**2 + r0 * (-2 * M + r0)) ** 1.5
        * 3.0
        * (m_max - 0.5)
        * (m_max + 0.5)
...
def assemble_scalar_self_force(reduction_files, r0, a):
    ...
    for m_mode, reduction_file in enumerate(reduction_files):
        with h5py.File(reduction_file, "r") as open_h5_file:
            ...
            df.loc[m_mode, "SelfForce_r"] = (
                to_dataframe(open_h5_file["SelfForce.dat"])
                .groupby("IterationId")
                .mean()["Re(SelfForce_r)"]
                .iloc[-1]
            )
    ...
    self_force += sum_Fr4(r0=r0, a=a, m_max=m_max)
