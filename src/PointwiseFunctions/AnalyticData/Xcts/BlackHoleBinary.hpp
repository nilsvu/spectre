...
struct IsolatedSolution;
template <>
struct IsolatedSolution<BackgroundSpacetime::FlatMaximallySliced> {
  using type = Xcts::Solutions::Schwarzschild<
      Xcts::Solutions::SchwarzschildCoordinates::Isotropic>;
};
template <>
struct IsolatedSolution<BackgroundSpacetime::FlatKerrSchildIsotropicSliced> {
  using type = Xcts::Solutions::Schwarzschild<
      Xcts::Solutions::SchwarzschildCoordinates::KerrSchildIsotropic>;
};
template <>
struct IsolatedSolution<BackgroundSpacetime::SuperposedKerrSchild> {
  using type =
      Xcts::Solutions::Kerr<Xcts::Solutions::KerrCoordinates::KerrSchild>;
};
template <>
struct IsolatedSolution<BackgroundSpacetime::SuperposedHarmonic> {
  using type =
      Xcts::Solutions::Kerr<Xcts::Solutions::KerrCoordinates::Harmonic>;
};
}  // namespace detail
...
class BlackHoleBinary {
 public:
  ...
  auto variables(
      const tnsr::I<DataType, 3, Frame::Inertial>& x,
      tmpl::list<
          Xcts::Tags::ConformalFactor<DataType>> /*meta*/)
      const noexcept -> tuples::TaggedTuple<
  ...
  template <typename DataType>
  auto variables(const tnsr::I<DataType, 3, Frame::Inertial>& x,
                 tmpl::list<Xcts::Tags::ShiftStrain<
                     DataType, 3, Frame::Inertial>> /*meta*/) const noexcept
      -> tuples::TaggedTuple<
 ...
 private:
  ...
  std::array<typename detail::IsolatedSolution<Background>::type, 2>
      isolated_solutions_;
  Xcts::Solutions::Flatness vacuum_{};
