...
CircularOrbit::variables(const tnsr::I<DataVector, 2>& x,
  ...
  // Compute boost function and its derivative
  auto& H = get<Tags::BoostFunction>(result);
  auto& dH = get<Tags::BoostFunctionDeriv>(result);
  if (hyperboloidal_slicing_transitions_.has_value()) {
    std::tie(get(H), get(dH)) = boost_function_and_deriv<2>(
        r_star_or_r, hyperboloidal_slicing_transitions_.value());
  } else {
    get(H) = make_with_value<ComplexDataVector>(r_star_or_r, 0.);
    get(dH) = make_with_value<ComplexDataVector>(r_star_or_r, 0.);
  }

  // Compute coefficients alpha, beta, gamma
  ...
  for (size_t p = 0; p < get(beta).size(); ++p) {
    ...
    get(beta)[p] =
        square(k) *
            (square(get(H)[p]) - sigma_squared[p] / r_sq_plus_a_sq_sq[p]) +
        2. * a * m_mode_number_ * k *
            (2. * M * r[p] / r_sq_plus_a_sq[p] + get(H)[p]) / r_sq_plus_a_sq[p];
    if (penetrating_horizon_) {
