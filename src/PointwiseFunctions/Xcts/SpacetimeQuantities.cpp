...
void SpacetimeQuantitiesComputer::operator()(
  ...
  TensorExpressions::evaluate(
      ...
      4. * conformal_laplacian_of_conformal_factor() -
          conformal_factor() * conformal_ricci_scalar() / 2. -
          pow<5>(conformal_factor()) * square(trace_extrinsic_curvature()) /
              2. +
          pow<5>(conformal_factor()) * inv_spatial_metric(ti_I, ti_K) *
              inv_spatial_metric(ti_J, ti_L) * extrinsic_curvature(ti_i, ti_j) *
              extrinsic_curvature(ti_k, ti_l) / 2. +
          8. * M_PI * pow<5>(conformal_factor()) * energy_density());
}
