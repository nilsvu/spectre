...
KerrSchild::KerrSchild(const double mass,
                       ...
                       const std::array<double, volume_dim> boost_velocity,
...
void KerrSchild::IntermediateComputer<DataType, Frame>::operator()(
    ...
    const gsl::not_null<CachedBuffer*> cache,
    internal_tags::deriv_H_unboosted<DataType, Frame> /*meta*/) const noexcept {
...
void KerrSchild::IntermediateComputer<DataType, Frame>::operator()(
  ...
  const auto& deriv_H =
      cache->get_var(internal_tags::deriv_H_unboosted<DataType, Frame>{});
...
void KerrSchild::IntermediateComputer<DataType, Frame>::operator()(
    ...
    const gsl::not_null<CachedBuffer*> cache,
    internal_tags::null_form_unboosted<DataType, Frame> /*meta*/) const noexcept {
...
void KerrSchild::IntermediateComputer<DataType, Frame>::operator()(
  ...
  const auto& null_form =
      cache->get_var(internal_tags::null_form_unboosted<DataType, Frame>{});
...
KerrSchild::IntermediateVars<DataType, Frame>::get_var(
  ...
  const auto& H = get_var(internal_tags::H<DataType>{});
  ...
  const auto& deriv_null_form =
      get_var(internal_tags::deriv_null_form<DataType, Frame>{});
...
KerrSchild::IntermediateVars<DataType, Frame>::get_var(
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    for (size_t j = i; j < 3; ++j) {  // Symmetry
      result.get(i, j) -=
          2.0 * H * lapse_squared * null_form.get(i + 1) * null_form.get(j + 1);
    }
  }
