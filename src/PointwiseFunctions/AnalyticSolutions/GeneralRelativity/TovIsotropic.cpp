...
TovIsotropic::RadialVariables<DataType> origin_solution(
    const EquationsOfState::EquationOfState<true, 1>& equation_of_state,
    const double central_conformal_factor,
    const double central_log_specific_enthalpy,
    const double log_lapse_at_outer_radius,
    const DataType& used_for_size) noexcept {
  const auto zero = make_with_value<DataType>(used_for_size, 0.);
  const auto small_number = make_with_value<DataType>(used_for_size, 1.e-15);
  TovIsotropic::RadialVariables<DataType> result{small_number};
  result.conformal_factor = Scalar<DataType>{
      make_with_value<DataType>(used_for_size, central_conformal_factor)};
  result.dr_conformal_factor = Scalar<DataType>{zero};
  result.specific_enthalpy = Scalar<DataType>{make_with_value<DataType>(
      used_for_size, exp(central_log_specific_enthalpy))};
  ...
  result.lapse = Scalar<DataType>{make_with_value<DataType>(
      used_for_size,
      exp(log_lapse_at_outer_radius - central_log_specific_enthalpy))};
  result.dr_lapse = Scalar<DataType>{zero};
  return result;
}
...
TovIsotropic::TovIsotropic(
  ...
  central_conformal_factor_ = 1. / sqrt(matching_constant);
...
double TovIsotropic::total_mass() const noexcept { return total_mass_; }
...
TovIsotropic::RadialVariables<double> TovIsotropic::radial_variables(
  ...
  } else if (radius < 1.e-14 * outer_radius_) {
    return origin_solution(equation_of_state, central_conformal_factor_,
                           areal_solution_.log_specific_enthalpy(0.),
                           log_lapse_at_outer_radius_,
                           std::numeric_limits<double>::signaling_NaN());
  } else {
    return interior_solution(equation_of_state, radius,
                             conformal_factor(radius),
...
TovIsotropic::RadialVariables<DataVector> TovIsotropic::radial_variables(
  ...
  if (max(radius) < 1.e-14 * outer_radius_) {
    return origin_solution(
        equation_of_state, central_conformal_factor_,
        areal_solution_.log_specific_enthalpy(0.), log_lapse_at_outer_radius_,
        DataVector{num_points, std::numeric_limits<double>::signaling_NaN()});
  }
  if (min(radius) >= 1.e-14 * outer_radius_ and max(radius) <= outer_radius_) {
  ...
  for (size_t i = 0; i < num_points; i++) {
    ...
    auto radial_vars_at_r =
        (r <= outer_radius_
             ? (r < 1.e-14 * outer_radius_
                    ? origin_solution(
                          equation_of_state, central_conformal_factor_,
                          areal_solution_.log_specific_enthalpy(0.),
                          log_lapse_at_outer_radius_,
                          std::numeric_limits<double>::signaling_NaN())
                    : interior_solution(equation_of_state, r,
                                        conformal_factor(r),
                                        log_specific_enthalpy(r), mass(r),
                                        log_lapse_at_outer_radius_))
             : vacuum_solution(r, total_mass_));
    result.isotropic_radius[i] = radial_vars_at_r.isotropic_radius;
