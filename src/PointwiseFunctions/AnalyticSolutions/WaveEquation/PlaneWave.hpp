...
class PlaneWave : public evolution::protocols::AnalyticSolution {
 public:
  ...
  tuples::TaggedTuple<ScalarWave::Phi<Dim>> variables(
      const tnsr::I<DataVector, Dim>& x, double t,
  ...
  tuples::TaggedTuple<Tags::dt<ScalarWave::Pi>> variables(
      const tnsr::I<DataVector, Dim>& x, double t,
      tmpl::list<Tags::dt<ScalarWave::Pi>> /*meta*/) const noexcept;

  tuples::TaggedTuple<Tags::dt<ScalarWave::Phi<Dim>>> variables(
      const tnsr::I<DataVector, Dim>& x, double t,
      tmpl::list<Tags::dt<ScalarWave::Phi<Dim>>> /*meta*/) const noexcept;

  tuples::TaggedTuple<Tags::dt<ScalarWave::Psi>> variables(
      const tnsr::I<DataVector, Dim>& x, double t,
