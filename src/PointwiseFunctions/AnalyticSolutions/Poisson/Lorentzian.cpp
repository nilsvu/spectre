...
template <>
tuples::TaggedTuple<::Tags::FixedSource<Field>> Lorentzian<3>::variables(
    const tnsr::I<DataVector, 3>& x,
    tmpl::list<::Tags::FixedSource<Field>> /*meta*/) noexcept {
  return {Scalar<DataVector>(3. / pow<5>(sqrt(1. + get(dot_product(x, x)))))};
}

template <>
tuples::TaggedTuple<::Tags::FixedSource<AuxiliaryField<3>>>
Lorentzian<3>::variables(
    const tnsr::I<DataVector, 3>& x,
    tmpl::list<::Tags::FixedSource<AuxiliaryField<3>>> /*meta*/) noexcept {
