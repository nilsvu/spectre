 ...
 * \brief A cubic spline interpolation class
 *
 * The class builds a cubic spline interpolant using the `x_values` and
 * `y_values` passed into the constructor. For details on the algorithm see
 * the GSL documentation on `gsl_interp_cspline`.
 *
 * Here is an example how to use this class:
 *
 * \snippet Test_CubicSpline interpolate_example
 */
class CubicSpline {
 public:
 ...
 private:
  struct gsl_interp_accel_deleter {
    void operator()(gsl_interp_accel* acc) noexcept;
  };
  struct gsl_spline_deleter {
    void operator()(gsl_spline* spline) noexcept;
  };
