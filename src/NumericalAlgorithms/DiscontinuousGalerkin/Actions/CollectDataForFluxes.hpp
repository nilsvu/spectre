 ...
 * on mortars.
 *
 * Set the `DirectionsTag` template parameter to `::Tags::InternalDirections` to
 * collect data on internal element interfaces, so it can be communicated to the
 * element's neighbors in a subsequent call to `dg::Actions::SendDataForFluxes`.
 *
 * Alternatively, set the `DirectionsTag` to
 * `::Tags::BoundaryDirectionsInterior` to collect data on external element
 * boundaries for imposing boundary conditions through numerical fluxes. In this
 * case, make sure mortars on external boundaries are initialized e.g. by
 * `dg::Actions::InitializeMortars`. Also, make sure the field data on exterior
 * ("ghost") element boundaries (see `::Tags::BoundaryDirectionsExterior`) has
 * been updated to represent the boundary conditions before invoking this
 * action.
 *
 * Design decisions:
 *
 ...
 * - Nomenclature: "Boundary data" is data collected on an element interface
...
struct CollectDataForFluxes<
    BoundaryScheme, ::Tags::InternalDirections<BoundaryScheme::volume_dim>> {
 private:
 ...
 public:
  ...
  static std::tuple<db::DataBox<DbTags>&&> apply(
    ...
    auto boundary_data_on_interfaces =
        interface_apply<boundary_data_computer,
    ...
    const auto& mortar_meshes =
        get<Tags::Mortars<Tags::Mesh<volume_dim - 1>, volume_dim>>(box);
    ...
    for (const auto& direction_and_neighbors : element.neighbors()) {
      ...
      }  // loop over direction_and_neighbors.second
...
template <typename BoundaryScheme>
struct CollectDataForFluxes<BoundaryScheme, ::Tags::BoundaryDirectionsInterior<
