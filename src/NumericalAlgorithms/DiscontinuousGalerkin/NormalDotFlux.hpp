...
void normal_dot_flux(
  ...
  if (result->number_of_grid_points() != fluxes.number_of_grid_points()) {
    result->initialize(fluxes.number_of_grid_points(), 0.);
  }
...
template <typename Tag, typename FluxTag, size_t VolumeDim, typename Fr>
struct NormalDotFluxCompute2 : db::add_tag_prefix<NormalDotFlux, Tag>,
                               db::ComputeTag {
 private:
  using normal_tag =
      Tags::Normalized<domain::Tags::UnnormalizedFaceNormal<VolumeDim, Fr>>;
 ...
 public:
  using base = db::add_tag_prefix<NormalDotFlux, Tag>;
  using return_type = typename base::type;
  using argument_tags = tmpl::list<FluxTag, normal_tag>;
  static void function(
      const gsl::not_null<return_type*> result,
      const typename FluxTag::type& flux,
      const tnsr::i<DataVector, VolumeDim, Fr>& normal) noexcept {
    normal_dot_flux(result, normal, flux);
  }
};
}  // namespace Tags
