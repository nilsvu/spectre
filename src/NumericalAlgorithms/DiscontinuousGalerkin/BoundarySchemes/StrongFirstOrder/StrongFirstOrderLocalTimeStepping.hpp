...
struct StrongFirstOrderLocalTimeStepping {
 ...
 public:
  ...
  using RemoteData = typename base::RemoteData;
  using remote_data_computer = typename base::remote_data_computer;

  using LocalData = typename base::LocalData;
  using local_data_computer = typename base::local_data_computer;

  using mortar_data_tag =
      Tags::BoundaryHistory<LocalData, RemoteData,
  ...
  static void apply(
    ...
    for (auto& mortar_id_and_data : *all_mortar_data) {
      ...
      const auto coupling = [
        &face_mesh, &mortar_mesh, &mortar_size, &extent_perpendicular_to_face,
        &normal_dot_numerical_flux_computer
      ](const LocalData& local_data, const RemoteData& remote_data) noexcept {
