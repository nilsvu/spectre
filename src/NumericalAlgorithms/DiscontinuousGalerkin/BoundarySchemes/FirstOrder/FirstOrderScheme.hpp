...
template <
    size_t Dim, typename VariablesTag, typename NumericalFluxComputerTag,
    typename NumericalFlux = db::const_item_type<NumericalFluxComputerTag>,
    typename ArgsTagsList = typename NumericalFlux::argument_tags,
    typename VolumeArgsTagsList = get_volume_tags<NumericalFlux>,
    typename PackageFieldTagsList = typename NumericalFlux::package_field_tags,
    typename PackageExtraTagsList = typename NumericalFlux::package_extra_tags>
...
struct boundary_data_computer_impl<
    Dim, VariablesTag, NumericalFluxComputerTag, NumericalFlux,
    tmpl::list<ArgsTags...>, tmpl::list<VolumeArgsTags...>,
  ...
  static constexpr auto apply = dg::FirstOrderScheme::package_boundary_data<
      Dim - 1, NumericalFlux, db::const_item_type<ArgsTags>...>;
};
...
struct FirstOrderScheme {
  ...
  static void apply(
    ...
    for (auto& mortar_id_and_data : *all_mortar_data) {
      ...
      const auto& magnitude_of_face_normal = [&face_normal_magnitudes_internal,
                                              &face_normal_magnitudes_boundary,
                                              &mortar_id]() {
        return mortar_id.second == ElementId<Dim>::external_boundary_id()
                   ? face_normal_magnitudes_boundary.at(mortar_id.first)
                   : face_normal_magnitudes_internal.at(mortar_id.first);
      }();
