...
Gmres<VarsType, Preconditioner, LinearSolverRegistrars>::solve(
  ...
  while (not has_converged) {
    ...
    // is also zero.
    if (alg::all_of(initial_guess, [](const double element) noexcept {
          return equal_within_roundoff(element, 0.);
        })) {
      initial_operand = make_with_value<VarsType>(initial_guess, 0.);
    } else {
      std::apply(
          linear_operator,
