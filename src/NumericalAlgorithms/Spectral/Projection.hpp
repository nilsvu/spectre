 ...
 * basis functions on the mesh, then pass `true` for the parameter `massive`
 * (default is `false`). The restriction operator for this case is just the
 * transpose of the prolongation operator, i.e. just an interpolation matrix
 * transpose. Note that the "massive" residual already takes the difference
 * in element size between parent and children into account by including a
 * Jacobian in the volume element of the integral.
 *
