...
struct ReadAllVolumeDataAndDistribute {
  ...
  static void apply(DataBox& box, Parallel::GlobalCache<Metavariables>& cache,
    ...
    const std::string files_dir = file_system::get_parent_path(filename_prefix);
    const std::string filename_prefix_no_dir =
        file_system::get_file_name(filename_prefix);
    const std::vector<std::string> all_files = file_system::ls(files_dir);
    bool found_a_file = false;
    for (const std::string& filename : all_files) {
      if (filename.rfind(filename_prefix_no_dir, 0) != 0) {
        continue;
      }
      found_a_file = true;

      // Open the volume data file
      h5::H5File<h5::AccessType::ReadOnly> h5file(files_dir + "/" + filename);
      ...
      for (auto& element_and_name : get<Tags::RegisteredElements>(box)) {
        if (std::find(all_grid_names.begin(), all_grid_names.end(),
                      element_and_name.second) == all_grid_names.end()) {
          continue;
        }
    ...
    if (not found_a_file) {
      ERROR_NO_TRACE("No file with the prefix '" << filename_prefix
                                                 << "' was found.");
    }
  }
};
