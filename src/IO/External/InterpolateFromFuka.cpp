...
std::array<std::vector<double>, 16> KadathExportBH(
    ...
    char const* fn, double const interpolation_offset, int const interp_order,
...
std::array<std::vector<double>, 22> KadathExportNS(int const npoints,
                                                   double const* xx,
                                                   double const* yy,
                                                   double const* zz,
                                                   char const* fn);

std::array<std::vector<double>, 22> KadathExportBNS(int const npoints,
                                                    double const* xx,
                                                    double const* yy,
                                                    double const* zz,
                                                    char const* fn);
...
tuples::tagged_tuple_from_typelist<fuka_tags<IdType>> interpolate_from_fuka(
  ...
  const auto fuka_data = [&x, &info_filename, &interpolation_offset,
    ...
    if constexpr (IdType == FukaIdType::Bh) {
      return KadathExportBH(x.begin()->size(), get<0>(x).data(),
                            get<1>(x).data(), get<2>(x).data(),
                            info_filename.c_str(), interpolation_offset,
                            interp_order, delta_r_rel);
    } else if constexpr (IdType == FukaIdType::Bbh) {
      return KadathExportBBH(x.begin()->size(), get<0>(x).data(),
                             get<1>(x).data(), get<2>(x).data(),
                             info_filename.c_str(), interpolation_offset,
                             interp_order, delta_r_rel);
    } else if constexpr (IdType == FukaIdType::Ns) {
      return KadathExportNS(x.begin()->size(), get<0>(x).data(),
                            get<1>(x).data(), get<2>(x).data(),
                            info_filename.c_str());
    } else if constexpr (IdType == FukaIdType::Bns) {
      return KadathExportBNS(x.begin()->size(), get<0>(x).data(),
                             get<1>(x).data(), get<2>(x).data(),
                             info_filename.c_str());
    } else if constexpr (IdType == FukaIdType::Bhns) {
      return KadathExportBHNS(x.begin()->size(), get<0>(x).data(),
                              get<1>(x).data(), get<2>(x).data(),
                              info_filename.c_str(), interpolation_offset,
                              interp_order, delta_r_rel);
    } else {
