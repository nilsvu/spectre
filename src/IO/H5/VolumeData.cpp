            ...
            if (UNLIKELY(i == 0)) {
              ...
              if (dim == 2) {
                if (element.basis[0] == Spectral::Basis::SphericalHarmonic and
                  ...
                  for (size_t k = 0; k < (l + 1) * (2 * l + 1); ++k) {
                    if (k % (l + 1) == 0) {
                      top_pole_points.push_back(k);
                    }
                    if (k % (l + 1) == l) {
                      bottom_pole_points.push_back(k);
                    }
                  }
                  ...
                  const size_t number_of_points_near_poles =
                      top_pole_points.size();
                  size_t to_next_triangle_point = 1;
                  while (number_of_points_near_poles / to_next_triangle_point >=
                         3) {
                    for (size_t point_starting_triangle = 0;
                         point_starting_triangle <
                         number_of_points_near_poles -
                             2 * to_next_triangle_point;
                         point_starting_triangle +=
                         2 * to_next_triangle_point) {
                      ...
                      pole_connectivity.push_back(gsl::at(
                          top_pole_points, point_starting_triangle +
                                               2 * to_next_triangle_point));
                      pole_connectivity.push_back(
                      ...
                      pole_connectivity.push_back(gsl::at(
                          ...
                          point_starting_triangle + to_next_triangle_point));
                      pole_connectivity.push_back(gsl::at(
                          bottom_pole_points, point_starting_triangle +
                                                  2 * to_next_triangle_point));
                    }
                    ...
                    if (number_of_points_near_poles % 2 != 0 and
                        2 * to_next_triangle_point <
                            number_of_points_near_poles) {
                      pole_connectivity.push_back(gsl::at(
                          top_pole_points, number_of_points_near_poles -
                                               2 * to_next_triangle_point));
                      pole_connectivity.push_back(
                          gsl::at(top_pole_points, number_of_points_near_poles -
                                                       to_next_triangle_point));
                      pole_connectivity.push_back(gsl::at(top_pole_points, 0));
                      pole_connectivity.push_back(gsl::at(
                          bottom_pole_points, number_of_points_near_poles -
                                                  2 * to_next_triangle_point));
                      pole_connectivity.push_back(gsl::at(
                          bottom_pole_points, number_of_points_near_poles -
                                                  to_next_triangle_point));
                      pole_connectivity.push_back(
                          gsl::at(bottom_pole_points, 0));
                    }
...
std::vector<std::string> VolumeData::list_tensor_components(
  ...
  const auto last = tensor_components.end();
  const long number_of_components_to_remove = search_result != last ? 6 : 5;
