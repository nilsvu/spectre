...
enum class ItemType { Mutable, Compute, Reference };
...
class Item<Tag, Requires<db::is_mutable_item_tag_v<Tag>>> {
 public:
  ...
  void pack_unpack(PUP::er& p) noexcept { p | value_; }
...
template <typename Tag>
class Item<Tag, Requires<db::is_compute_tag_v<Tag>>> {
 public:
  ...
  // NOLINTNEXTLINE(google-runtime-references)
  void pack_unpack(PUP::er& p) noexcept {
    p | evaluated_;
    if (evaluated_) {
      p | value_;
    }
...
template <typename Tag>
class Item<Tag, Requires<db::is_reference_tag_v<Tag>>> {
 public:
