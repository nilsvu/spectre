...
void bind_dense_matrix(py::module& m) {  // NOLINT
  using MatrixType = DenseMatrix<double>;
  py::class_<MatrixType>(m, "DenseMatrix", py::buffer_protocol())
      ...
      .def(
          "__getitem__",
          +[](const MatrixType& self, const std::tuple<size_t, size_t>& x) {
      ...
      .def(
          "__setitem__",
          +[](MatrixType& self, const std::tuple<size_t, size_t>& x,
              const double val) {
            matrix_bounds_check(self, std::get<0>(x), std::get<1>(x));
