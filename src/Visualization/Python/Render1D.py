...
#The module name below depends on python version used to run the script
#Imports Tkinter for python2, tkinter for python3
try:
...
def parse_cmd_line():
    ...
    group_vars.add_argument(
        '--list-vars',
        type=bool,
        help="If true then print to screen vars in h5 file "
        "and exit")
    parser.add_argument(
        '--time',
        type=int,
    ...
    parser.add_argument('-o',
                        ...
                        "file extension. If not specified, renders to screen.")
    ...
    # Check for h5 suffix if '--file-prefix' argument used
...
def render_single_time(var_name, time_slice, output_prefix, coords, data):
    '''
    ...
    plt.xlabel("x")
    plt.ylabel(var_name)
    plt.title("t = {:.5f}".format(int(time_slice)))
    plt.plot(coords[int(time_slice)], data[int(time_slice)], 'o')
    if output_prefix:
...
def render_animation(var_name, output_prefix, interval, time, coords, data):
    ...
    if output_prefix:
        ...
        # Provides an error message with traceback when an exception is raised
...
if __name__ == "__main__":
    ...
    if args['list_vars']:
        print_var_names(files)
        sys.exit(0)
    time, coords, data = get_data(files, args['var'])
    ...
    if args['time'] is None:
        ...
        render_single_time(args['var'], args['time'], args['output'], coords,
                           data)
