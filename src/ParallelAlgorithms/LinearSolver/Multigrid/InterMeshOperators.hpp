...
// Note that only to "massless" fields restrict with `R`, i.e.
// polynomial approximations on the grid, and not "massive" quantities, i.e.
// DG residuals that involve an integral over basis functions and thus an
// application of the mass matrix. The latter can be restricted by simply
// applying the interpolation matrix transpose I_coarse_to_fine^T (see
