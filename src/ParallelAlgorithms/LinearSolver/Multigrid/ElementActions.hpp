...
struct InitializeElement {
  ...
  using simple_tags =
      tmpl::list<Tags::MultigridLevel, Tags::IsFinestGrid, Tags::ParentId<Dim>,
                 Tags::ChildIds<Dim>, Tags::ParentMesh<Dim>,
                 observers::Tags::ObservationKeySuffix<Tags::MultigridLevel>,
                 observers::Tags::ObservationKeySuffix<Tags::IsFinestGrid>,
                 Tags::ObservationId<OptionsGroup>,
                 Tags::VolumeDataForOutput<OptionsGroup, FieldsTag>>;
  using compute_tags = tmpl::list<>;
  using const_global_cache_tags =
      tmpl::list<Tags::OutputVolumeData<OptionsGroup>>;
  ...
  static std::tuple<db::DataBox<DbTagsList>&&> apply(
    ...
    // Validate options
    if (db::get<Tags::MaxLevels<OptionsGroup>>(box) == 0) {
      ERROR_NO_TRACE(
          "The 'MaxLevels' option includes the finest grid, so '0' is not a "
          "valid value. Set the option to '1' to disable multigrid.");
    }
    if (db::get<Convergence::Tags::Iterations<OptionsGroup>>(box) == 0 and
        db::get<Tags::MaxLevels<OptionsGroup>>(box) != 1) {
      ERROR_NO_TRACE(
          "You disabled multigrid by requesting it runs no iterations. Also "
          "set 'MaxLevels' to '1' to avoid creating unused multigrid levels.");
    }
    ...
    std::unordered_set<ElementId<Dim>> child_ids = multigrid::child_ids(
        element_id,
        db::get<Tags::ChildrenRefinementLevels<Dim>>(
    ...
    Initialization::mutate_assign<simple_tags>(
        make_not_null(&box), multigrid_level, is_finest_grid,
        std::move(parent_id), std::move(child_ids), std::move(parent_mesh),
        std::move(observation_key_level),
...
struct PrepareSolve {
 ...
 public:
  ...
  apply(db::DataBox<DbTagsList>& box,
    ...
    // Pause the algorithm on coarser grids. This allows the finest grid to
    // control the algorithm. The `SendResidualToCoarserGrid` action will
    // continue the algorithm on the coarser grids whenever they receive data.
    // We make sure to not pause the algorithm if data has already been received
    // from children, because that means the algorithm should certainly
    // continue.
    const bool pause =
        (not db::get<Tags::IsFinestGrid>(box)) and
        (not(tuples::get<DataFromChildrenInboxTag<Dim, tmpl::list<SourceTag>>>(
                 inboxes)
                 .count(0) > 0));
    return {std::move(box), pause ? Parallel::AlgorithmExecution::Pause
...
struct PreparePreSmoothing {
 ...
 public:
  ...
  static std::tuple<db::DataBox<DbTagsList>&&> apply(
    ...
    if (not db::get<Tags::IsFinestGrid>(box)) {
...
struct ReceiveCorrectionFromCoarserGrid {
 ...
 public:
  ...
  apply(db::DataBox<DbTagsList>& box,
    ...
    const auto prolongated_parent_correction = [&parent_correction,
                                                &parent_mesh, &mesh,
                                                &child_size]() noexcept {
