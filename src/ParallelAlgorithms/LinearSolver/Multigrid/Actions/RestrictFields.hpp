...
namespace LinearSolver::multigrid::Actions {
...
struct SendFieldsToCoarserGrid {
  ...
  static std::tuple<db::DataBox<DbTagsList>&&> apply(
    ...
    auto fields = db::get<FieldsTag>(box);
    ...
    auto restricted_fields = apply_matrices(
        ...
        db::item_type<ReceiveTag>(std::move(fields)),
...
struct ReceiveFieldsFromFinerGrid {
  ...
  static std::tuple<db::DataBox<DbTagsList>&&> apply(
    ...
    db::mutate<ReceiveTag>(
        make_not_null(&box),
        [&children_data](const gsl::not_null<db::item_type<ReceiveTag>*> source,
                         const Mesh<Dim>& mesh,
                         const Scalar<DataVector>& det_jacobian) noexcept {
          *source = db::item_type<ReceiveTag>{mesh.number_of_grid_points(), 0.};
