...
struct NormalizeInitialOperand
    : Parallel::Actions::Receive<
          Tags::InitialOrthogonalization<OptionsGroup>,
...
struct OrthogonalizeOperand
    : Parallel::Actions::Receive<
          Tags::Orthogonalization<OptionsGroup>,
          LinearSolver::Tags::IterationId<OptionsGroup>> {
 private:
  using fields_tag = FieldsTag;
  using operand_tag =
 ...
 public:
  ...
  static std::tuple<db::DataBox<DbTagsList>&&, bool, size_t> apply(
      ...
      const ArrayIndex& array_index, const ActionList /*meta*/,
      const ParallelComponent* const /*meta*/) noexcept {
    const double orthogonalization = Parallel::extract_from_inbox<
        Tags::Orthogonalization<OptionsGroup>,
        LinearSolver::Tags::IterationId<OptionsGroup>>(inboxes, box);
...
struct NormalizeOperandAndUpdateField
    : Parallel::Actions::Receive<
          Tags::FinalOrthogonalization<OptionsGroup>,
          LinearSolver::Tags::IterationId<OptionsGroup>> {
 private:
  using fields_tag = FieldsTag;
  using initial_fields_tag =
