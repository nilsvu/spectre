...
struct PrepareSolve {
 private:
  ...
  using preconditioned_operand_tag =
      db::add_tag_prefix<LinearSolver::Tags::Preconditioned, fields_tag>;
    ...
    db::mutate<LinearSolver::Tags::IterationId<OptionsGroup>,
        ...
        [](const gsl::not_null<size_t*> iteration_id,
          ...
          *orthogonalization_iteration_id = 0;
          // Always start preconditioner with an initial guess of zero because
          // we have no reason to expect the remaining residual to have a
          // particular form
          //   *preconditioned_operand =
