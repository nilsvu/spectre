...
struct ElementCenteredSubdomainData {
  ...
  ElementCenteredSubdomainDataIterator<false, Dim, TagsList> begin() noexcept {
    return {this};
  }

  ElementCenteredSubdomainDataIterator<false, Dim, TagsList> end() noexcept {
    return {};
  }

  ElementCenteredSubdomainDataIterator<true, Dim, TagsList> begin() const
      noexcept {
    return {this};
  }

  ElementCenteredSubdomainDataIterator<true, Dim, TagsList> end() const
      noexcept {
    return {};
  }
...
/// If you need this for anything performance-critical you should do the
/// work of implementing non-owning variables
template <bool Const, size_t Dim, typename TagsList>
struct ElementCenteredSubdomainDataIterator {
 private:
  using PtrType =
 ...
 public:
  ...
  ElementCenteredSubdomainDataIterator(PtrType data) noexcept : data_(data) {
    overlap_ids_ = ordered_overlap_ids(data_->overlap_data);
    reset();
  }
 ...
 private:
  template <bool LocalConst, size_t LocalDim, typename LocalTagsList>
  friend bool operator!=(
      const ElementCenteredSubdomainDataIterator<LocalConst, LocalDim,
                                                 LocalTagsList>& lhs,
      const ElementCenteredSubdomainDataIterator<LocalConst, LocalDim,
                                                 LocalTagsList>& rhs) noexcept {
