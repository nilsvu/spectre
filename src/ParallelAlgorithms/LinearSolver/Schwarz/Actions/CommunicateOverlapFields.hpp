 ...
 * elements. The `RestrictToOverlap` flag controls whether the tags are simply
 * retrieved from the element and sent as-is (`false`) or only the data that
 * intersect the overlap region are sent (`false`). If `RestrictToOverlap` is
...
struct SendOverlapFields {
  ...
  static std::tuple<db::DataBox<DbTagsList>&&> apply(
    ...
    for (const auto& direction_and_neighbors : element.neighbors()) {
      ...
      tuples::tagged_tuple_from_typelist<OverlapFields> overlap_fields{};
      if constexpr (RestrictToOverlap) {
        ...
        const size_t intruding_extent =
            gsl::at(get<Tags::IntrudingExtents<Dim, OptionsGroup>>(box),
                    direction.dimension());
        tmpl::for_each<OverlapFields>([&overlap_fields, &box, &element_extents,
                                       &intruding_extent,
                                       &direction](const auto tag_v) noexcept {
          using tag = tmpl::type_from<decltype(tag_v)>;
          get<tag>(overlap_fields) = LinearSolver::Schwarz::data_on_overlap(
              db::get<tag>(box), element_extents, intruding_extent, direction);
        });
      } else {
        tmpl::for_each<OverlapFields>(
      ...
      for (auto neighbor = neighbors.begin(); neighbor != neighbors.end();
        ...
        Parallel::receive_data<
            detail::OverlapFieldsTag<Dim, OverlapFields, OptionsGroup>>(
  ...
  static std::tuple<db::DataBox<DbTagsList>&&> apply(
    ...
    db::mutate<Tags::Overlaps<OverlapFields, Dim, OptionsGroup>...>(
        make_not_null(&box), [&received_overlap_fields](
          ...
          for (auto& overlap_id_and_fields : received_overlap_fields) {
            ...
            const auto helper = [&overlap_id](const auto local_overlap_field,
              ...
              (*local_overlap_field)[overlap_id] = std::move(overlap_field);
            };
