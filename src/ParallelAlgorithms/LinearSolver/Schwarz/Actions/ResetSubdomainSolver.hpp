 ...
 *
 * Use this action when the linear operator changed, e.g. when the nonlinear
 * solver updates the fields which constitute background fields for the
 * linearized operator.
 *
 ...
 * means that caches built up for the initial linear operator are never cleared,
 * so expensive re-initializations are avoided but the subdomain solves may be
 ...
 * optimization would be to make the choice whether or not to reset the
 * subdomain solver a runtime decision.
 */
