 ...
 *
 * Invoke this action when the linear operator changed. For example, an operator
 * representing the linearization of a nonlinear operator changes whenever the
 * point around which it is being linearized gets updated, i.e. in every
 * iteration of a nonlinear solve. Note that the operator does _not_ change
 * between iterations of a linear solve, so make sure you place this action
 * _outside_ the looping action list for the linear solve.
 *
