 ...
 *
 * For boundary schemes that communicate asynchronously (e.g. for local
 * time-stepping) see the documentation of this action's template
 * specialization.
 *
 * Uses:
 * - DataBox:
...
struct ReceiveDataForFluxes {
 ...
 public:
  ...
  static std::tuple<db::DataBox<DbTags>&&> apply(
      ...
      const ArrayIndex& /*array_index*/, const ActionList /*meta*/,
      const ParallelComponent* const /*meta*/) noexcept {
    if (get<domain::Tags::Element<volume_dim>>(box).number_of_neighbors() ==
        0) {
      return {std::move(box)};
    }
