 ...
 * initializes the `BoundaryScheme::receive_temporal_id` on mortars. It assumes
 * that the current state is _before_ any mortar communications take place and
 * the first communication will happen at what is currently the
 * `receive_temporal_id`.
 *
 * Uses:
 * - DataBox:
...
struct InitializeMortars {
 ...
 public:
  ...
  static auto apply(DataBox& box,
    ...
    using need_next_temporal_id_on_mortars = cpp17::negation<
 ...
 private:
  ...
  static auto initialize_next_temporal_id_on_mortars(
      db::DataBox<DbTagsList>&& box, std::true_type /* meta */) noexcept {
  ...
  static db::DataBox<DbTagsList> initialize_next_temporal_id_on_mortars(
      db::DataBox<DbTagsList>&& box, std::false_type /* meta */) noexcept {
    return std::forward<db::DataBox<DbTagsList>>(box);
  }
};
}  // namespace Actions
}  // namespace dg
