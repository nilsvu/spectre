...
class ObserveAtPoint<Dim, ObservationValueTag,
 ...
 public:
  ...
  /// \cond
  explicit ObserveAtPoint(CkMigrateMessage* msg) noexcept;
  ...
  ObserveAtPoint(const std::string& subfile_name,
                 ...
                 std::vector<std::string> tensor_names) noexcept
 ...
 private:
  static bool domain_is_time_dependent(const Domain<Dim>& domain) noexcept {
  ...
  template <typename DbTagsList>
  std::optional<tnsr::I<DataVector, Dim, Frame::Logical>>
  get_element_logical_coords(
      const db::DataBox<DbTagsList>& box) const noexcept {
    ...
    const auto block_logical_coords = [&coords_as_tensor, &box]() noexcept {
...
ObserveAtPoint<Dim, ObservationValueTag, tmpl::list<ObservableTensorTags...>,
               ArraySectionIdTag>::ObserveAtPoint(CkMigrateMessage*
                                                      msg) noexcept
    : Event(msg) {}
...
operator()(const typename ObservationValueTag::type& observation_value,
  ...
  tmpl::for_each<tensor_tags>([this, &box, &interpolant, &interpolated_data,
                               &component_names](auto tag_v) noexcept {
