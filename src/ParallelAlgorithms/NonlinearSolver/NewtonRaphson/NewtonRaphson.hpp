 ...
 * b-A_\mathrm{nonlinear}(x_k) \equiv b_\mathrm{linear}
 * \f}
 *
 * for the correction \f$\delta x_k\f$ and then updating the solution as
 * \f$x_{k+1}=x_k + \delta x_k\f$.
 *
 * The operations we need to supply to the algorithm is the nonlinear operator
 * \f$A_\mathrm{nonlinear}(x)\f$ and a linear solver for the linearized problem
 * \f$\frac{\delta A_\mathrm{nonlinear}}{\delta x}(x_k) \delta x_k =
 * b_\mathrm{linear}\f$. Each step of the algorithm expects
 * that \f$A_\mathrm{nonlinear}(x)\f$ is computed and stored in the DataBox as
 ...
 * \f$b_\mathrm{linear}\f$ (the `linear_solver_source_tag`). You will find
 * suitable iterative linear solvers in the `LinearSolver` namespace. The third
 * template parameter allows you to pass any further actions you wish to run in
 * each step of the algorithm (such as observations). If you add the `solve`
 * action list multiple times, use the fourth template parameter to label each
 * solve with a different type.
 *
 * \par Globalization:
 * This nonlinear solver supports a line-search (or "backtracking")
 * globalization. If a step does not to sufficiently decrease the residual (see
 ...
 * \cite DennisSchnabel (p. 325). Alternatives to a line-search globalization,
 * such as a trust-region globalization or more sophisticated nonlinear
 * preconditioning techniques (see e.g. \cite Brune2015 for an overview), are
