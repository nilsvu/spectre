...
struct NewtonRaphson {
  ...
  using initialize_element = tmpl::list<
      ...
      typename GlobalizationStrategy::initialize_element>;

  using prepare_solve = newton_raphson_detail::PrepareSolve;

  // Compile-time interface for observers
  using observed_reduction_data_tags = observers::make_reduction_data_tags<
      tmpl::list<observe_detail::reduction_data>>;

  using prepare_step = tmpl::list<newton_raphson_detail::PrepareStep,
                                  typename GlobalizationStrategy::prepare>;

  using update_residual =
      newton_raphson_detail::UpdateResidual<FieldsTag, GlobalizationStrategy>;
