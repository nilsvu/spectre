  ...
  static auto apply(db::DataBox<DbTagsList>& box,
    ...
    if constexpr (tmpl::size<typename system::background_fields>::value == 0) {
      ...
      auto background_fields =
          detail::all_background_fields<typename system::background_fields>(
      ...
      return std::make_tuple(std::move(
          ::Initialization::merge_into_databox<
              InitializeFixedSources, db::AddSimpleTags<::Tags::Variables<
                                          typename system::background_fields>>>(
              std::move(new_box), std::move(background_fields))));
    }
  }
};
