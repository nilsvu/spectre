...
class BoundaryConditionBase : public PUP::able {
 ...
 public:
  template <typename DbTagsList, size_t Dim, typename... FieldsAndFluxes>
  void operator()(const db::DataBox<DbTagsList>& box,
    ...
    call_with_dynamic_type<void, creatable_classes>(
        this, [&box, &direction,
          ...
          using Derived = tmpl::decay_t<decltype(boundary_condition)>;
          db::apply_at<
              tmpl::transform<
...
class BoundaryCondition : public BoundaryConditionBase<Dim, Registrars> {
 ...
 public:
  using linearization_registrars =
      tmpl::transform<typename Base::creatable_classes,
                      get_linearization_registrar>;
 ...
 public:
  ...
  explicit BoundaryCondition(CkMigrateMessage* m) noexcept : PUP::able(m) {}
  WRAPPED_PUPable_abstract(BoundaryCondition);  // NOLINT

 public:
  virtual const BoundaryConditionBase<Dim, linearization_registrars>&
  linearization() const noexcept = 0;
};
