...
class MinusLaplacian
 ...
 public:
  ...
  MinusLaplacian(StoredSolverType solver,
                 std::optional<elliptic::BoundaryConditionType>
 ...
 private:
  ...
  get_cached_boundary_conditions_signatures(
    ...
    if (not bc_signatures_.has_value()) {
      ...
      const auto collect_bc_signatures =
          [&bc_signatures = *bc_signatures_,
           &override_boundary_condition_type = boundary_condition_type_,
           &blocks](const BoundaryId& boundary_id) noexcept {
            ...
            ASSERT(bc_types.size() == num_components,
                   ...
                   "boundary condition in block "
                       << boundary_id.first << ", direction "
                       << boundary_id.second << " returned " << bc_types.size()
                       << " items, but expected " << num_components
                       << " (one for each tensor component).");
            for (size_t component = 0; component < num_components;
                 ++component) {
  ...
  get_cached_solver_and_boundary_conditions(
      ...
      const size_t component) const noexcept {
    if (bc_signatures.empty()) {
    ...
    const Solver& cached_solver = [this,
                                   &bc_signature]() noexcept -> const Solver& {
      if constexpr (std::is_abstract_v<Solver>) {
