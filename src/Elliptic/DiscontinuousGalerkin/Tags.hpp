...
struct Massive {
  using type = bool;
  static constexpr Options::String help =
      "Whether or not to multiply the DG operator with the mass matrix. Massive DG operators "
      "can be easier to solve because they are either symmetric, or at least closer to symmetry";
  using group = DiscontinuousGalerkin;
};
...
struct Massive : db::SimpleTag {
  ...
  static bool create_from_options(const bool value) noexcept {
    return value;
  }
};
