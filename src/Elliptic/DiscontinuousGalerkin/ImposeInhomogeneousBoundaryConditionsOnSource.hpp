...
struct ImposeInhomogeneousBoundaryConditionsOnSource {
  ...
  static auto compute_neumann_fluxes(
      const Variables<tmpl::list<AuxiliaryFields...>>& neumann_aux_fields,
      tmpl::list<PrimalFields...> /*meta*/,
      const FluxesComputer& fluxes_computer,
      const FluxesArgs&... fluxes_args) noexcept {
    using flux_tags =
        db::wrap_tags_in<::Tags::Flux, tmpl::list<PrimalFields...>,
                         tmpl::size_t<volume_dim>, Frame::Inertial>;
    Variables<flux_tags> neumann_fluxes{
        neumann_aux_fields.number_of_grid_points()};
    fluxes_computer.apply(
        make_not_null(&get<::Tags::Flux<PrimalFields, tmpl::size_t<volume_dim>,
                                        Frame::Inertial>>(neumann_fluxes))...,
        fluxes_args..., get<AuxiliaryFields>(neumann_aux_fields)...);
    return neumann_fluxes;
  }

  template <typename NormalDotNumericalFluxComputer,
            typename... NumericalFluxTags, typename... BoundaryDataTags,
            typename... FluxesArgs>
  static void compute_neumann_boundary_normal_dot_numerical_flux(
      const gsl::not_null<Variables<tmpl::list<NumericalFluxTags...>>*>
          numerical_fluxes,
      const NormalDotNumericalFluxComputer& normal_dot_numerical_flux_computer,
      const Variables<tmpl::list<BoundaryDataTags...>>& boundary_data,
      ...
      const FluxesType& fluxes_computer,
      const FluxesArgs&... fluxes_args) noexcept {
    normal_dot_numerical_flux_computer.compute_neumann_boundary(
        make_not_null(&get<NumericalFluxTags>(*numerical_fluxes))...,
          ...
          switch (boundary_condition) {
            case elliptic::BoundaryCondition::Dirichlet: {
              ...
              compute_dirichlet_boundary_normal_dot_numerical_flux(
                  ...
                  normalized_face_normal, magnitude_of_face_normal,
                  fluxes_computer, fluxes_args...);
              break;
            }
            case elliptic::BoundaryCondition::Neumann: {
              ...
              compute_neumann_boundary_normal_dot_numerical_flux(
                  make_not_null(&boundary_normal_dot_numerical_fluxes),
                  normal_dot_numerical_flux_computer,
                  std::move(neumann_boundary_data), normalized_face_normal,
                  fluxes_computer, fluxes_args...);
              break;
            }
          }
