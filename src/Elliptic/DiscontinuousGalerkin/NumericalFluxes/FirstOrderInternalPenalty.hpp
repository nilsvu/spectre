 ...
 * penalty is being computed on. \f$C\f$ is the \emph{penalty parameter}.
 *
 * \see `elliptic::dg::NumericalFluxes::FirstOrderInternalPenalty` for details
 */
DataVector penalty(const DataVector& element_size,
...
struct FirstOrderInternalPenalty<Dim, FluxesComputerTag,
 ...
 public:
  ...
  using package_extra_tags = tmpl::list<PerpendicularNumPoints, ElementSize>;
  ...
  void package_data(
      ...
      const gsl::not_null<DataVector*> element_size,
    ...
    *element_size = 2. / get(face_normal_magnitude);
  ...
  void operator()(
      ...
      const DataVector& element_size_interior,
      const db::item_type<::Tags::NormalDotFlux<
      ...
      const size_t perpendicular_num_points_exterior,
      const DataVector& element_size_exterior) const noexcept {
    const auto penalty = ::elliptic::dg::NumericalFluxes::penalty(
        min(element_size_interior, element_size_exterior),
