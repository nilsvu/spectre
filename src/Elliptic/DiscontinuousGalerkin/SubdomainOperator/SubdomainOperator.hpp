 ...
 * elements. It is a `LinearSolver::Schwarz::protocols::SubdomainOperator` to be
 * used with the Schwarz linear solver when it solves the elliptic DG operator.
 *
...
struct SubdomainOperator
 ...
 private:
  ...
  using apply_args_tags = tmpl::list<
      domain::Tags::Mesh<Dim>, domain::Tags::Coordinates<Dim, Frame::Inertial>,
      domain::Tags::InverseJacobian<Dim, Frame::Logical, Frame::Inertial>,
      domain::Tags::Interface<
          domain::Tags::InternalDirections<Dim>,
          ::Tags::Magnitude<domain::Tags::UnnormalizedFaceNormal<Dim>>>,
 ...
 public:
  ...
  void operator()(
    ...
    const auto apply_boundary_condition =
        [&box, &local_domain](const ElementId<Dim>& local_element_id,
          ...
          ASSERT(boundary_conditions.contains(local_direction),
                 "No boundary condition is available in block " +
                     std::to_string(local_element_id.block_id()) +
                     " in direction " + get_output(local_direction) +
          ...
          elliptic::apply_boundary_condition<
              true, tmpl::conditional_t<is_overlap_v, make_overlap_tag, void>>(
    ...
    const auto& local_central_element = central_element;
    const auto apply_boundary_condition_center =
        [&apply_boundary_condition, &local_central_element](
            const Direction<Dim>& local_direction,
            const auto... fields_and_fluxes) noexcept {
    ...
    for (const auto& [direction, neighbors] : central_element.neighbors()) {
      ...
      for (const auto& neighbor_id : neighbors) {
        ...
        const auto& local_neighbor_id = neighbor_id;
        const auto apply_boundary_condition_neighbor =
            [&apply_boundary_condition, &local_neighbor_id, &overlap_id](
                const Direction<Dim>& local_direction,
                const auto... fields_and_fluxes) noexcept {
