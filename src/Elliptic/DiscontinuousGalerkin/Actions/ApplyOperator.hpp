...
struct PrepareAndSendMortarData<
 ...
 public:
  ...
  static std::tuple<db::DataBox<DbTagsList>&&> apply(
    ...
    // advance. This should be safe since the DataBox is not manipulated while
    // the pointers are being used (no `db::create_from`, serialization etc.),
    // but we have to take care of propagating mutations to the data to subtimes
    // (see below).
    const auto [primal_fluxes, all_mortar_data] =
    ...
    elliptic::dg::prepare_mortar_data<System, Linearized>(
        ...
        make_not_null(&auxiliary_fluxes_buffer), primal_fluxes, all_mortar_data,
        db::get<PrimalFieldsTag>(box), element,
 ...
 public:
  ...
  static std::tuple<db::DataBox<DbTags>&&> apply(
    ...
    db::mutate<OperatorAppliedToFieldsTag, all_mortar_data_tag>(
        make_not_null(&box),
        [](const auto operator_applied_to_fields, const auto all_mortar_data,
           const auto& primal_fields, const auto& primal_fluxes,
           const auto&... args) noexcept {
          elliptic::dg::apply_operator<System, Linearized>(
              operator_applied_to_fields, all_mortar_data, primal_fields,
              primal_fluxes, args...);
        },
 ...
 * the `PrimalFieldsTag` and the `PrimalFluxesTag`.
 */
template <typename System, bool Linearized, typename TemporalIdTag,
          typename PrimalFieldsTag, typename PrimalFluxesTag,
