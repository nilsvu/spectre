...
struct DgOperatorImpl<System, Linearized, tmpl::list<PrimalFields...>,
  ...
  static void prepare_mortar_data(
      ...
      const std::function<bool(const ElementId<Dim>&)>& data_is_zero =
          [](const ElementId<Dim>& /*unused*/) noexcept { return false; },
    ...
    // const auto data_is_zero =
    //     [&elements_with_zero_data](const ElementId<Dim>& element_id) {
    //       return elements_with_zero_data.find(element_id) !=
    //              elements_with_zero_data.end();
    //     };
    ...
    if (AllDataIsZero or local_data_is_zero) {
      ...
      if constexpr (all_aux_vars_are_derivs) {
            ...
            [&auxiliary_fluxes,
             &primal_vars](const auto&... expanded_fluxes_args) noexcept {
              FluxesComputer::apply(
                  make_not_null(
                      &get<make_aux_flux_tag_t<AuxiliaryFields, PrimalFields,
                                               Dim>>(auxiliary_fluxes))...,
        ...
        std::apply(
            [&auxiliary_vars,
             &aux_derivs](const auto&... expanded_fluxes_args) noexcept {
              FluxesComputer::apply(
                  ...
                  get<tmpl::conditional_t<
                      ...
                      AuxiliaryFields,
                      ::Tags::deriv<AuxFluxTag<AuxiliaryFields>,
    ...
    for (const auto& direction : [&element]() noexcept -> const auto& {
      ...
      if (AllDataIsZero or local_data_is_zero) {
        ...
        if constexpr (all_aux_vars_are_derivs) {
          ...
          normal_times_flux(make_not_null(&n_dot_aux_fluxes), face_normal,
                            primal_vars_on_face);
        } else {
          ...
          normal_times_flux(make_not_null(&n_times_aux_on_face), face_normal,
                            auxiliary_fluxes_on_face);
          std::apply(
              [&n_dot_aux_fluxes, &n_times_aux_on_face](
                  const auto&... expanded_fluxes_args) noexcept {
                FluxesComputer::apply(
                    make_not_null(&get<::Tags::NormalDotFlux<AuxiliaryFields>>(
                        n_dot_aux_fluxes))...,
                    expanded_fluxes_args...,
                    get<tmpl::conditional_t<
                        is_deriv_tag_v<AuxiliaryFields, PrimalFields, Dim>,
      ...
      if (is_internal) {
        ...
        if constexpr (all_aux_vars_are_derivs) {
          ...
          std::apply(
              [&auxiliary_fluxes_on_face,
               &dirichlet_vars](const auto&... expanded_fluxes_args) noexcept {
                FluxesComputer::apply(
                    make_not_null(&get<make_aux_flux_tag_t<AuxiliaryFields,
                                                           PrimalFields, Dim>>(
                        auxiliary_fluxes_on_face))...,
                    expanded_fluxes_args...,
