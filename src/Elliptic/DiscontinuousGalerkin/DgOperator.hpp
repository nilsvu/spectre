...
template <typename TemporalId, typename PrimalVars, typename AuxiliaryVars>
struct MortarData
...
struct MortarData : db::SimpleTag {
  using type = elliptic::dg::MortarData<TemporalId, PrimalVars, AuxiliaryVars>;
};
}  // namespace Tags
...
struct DgOperatorImpl<System, Linearized, tmpl::list<PrimalFields...>,
  ...
  static void prepare_mortar_data(
    ...
    // Compute the auxiliary variables. They are the variables `v` in the
    // auxiliary equations `-div(F_v(u)) + S_v + v = 0` where `F_v` and `S_v`
    // are the auxiliary fluxes and sources.
  ...
  static void apply_operator(
    ...
    // variables and can therefore always be solved for the auxiliary variables
    // by just inverting the mass matrix. This Schur-complement formulation
    // avoids inflating the DG operator with the DOFs of the auxiliary
    // variables. In this form it is very similar to the "primal" formulation
    // where we get rid of the auxiliary variables through a DG theorem and thus
    // add the auxiliary boundary corrections _after_ computing the primal
    // fluxes. This involves a slightly different lifting operation with
    // differentiation matrices, which we avoid to implement for now by using
