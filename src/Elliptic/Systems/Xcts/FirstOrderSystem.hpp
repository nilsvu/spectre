 ...
 * ("conformal flatness"). In this case you can specify
 * `Xcts::Geometry::Euclidean` as the second template paramter so computations
 * are optimized for a flat background geometry and you don't have to supply
 * geometric background fields.
 */
template <Equations EnabledEquations, Geometry ConformalGeometry>
...
/// Linearization of the `Xcts::FirstOrderSystem`
template <Equations EnabledEquations, Geometry ConformalGeometry>
struct LinearizedFirstOrderSystem {
  ...
  // All fluxes are linear anyway, so we can re-use them
  using fluxes = Fluxes<EnabledEquations, ConformalGeometry>;
  using sources = LinearizedSources<EnabledEquations, ConformalGeometry>;

  using inv_metric_tag = typename nonlinear_system::inv_metric_tag;
  template <typename Tag>
