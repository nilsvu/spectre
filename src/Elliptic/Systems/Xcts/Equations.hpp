...
void longitudinal_shift(gsl::not_null<tnsr::II<DataType, 3>*> flux_for_shift,
...
void euclidean_longitudinal_shift(
    gsl::not_null<tnsr::II<DataType, 3>*> flux_for_shift,
    const tnsr::ii<DataType, 3>& shift_strain) noexcept;
...
struct Fluxes<Equations::HamiltonianLapseAndShift, Geometry::Euclidean> {
  ...
  static void apply(
    ...
    // Copy symmetric longitudinal shift into non-symmetric flux tensor.
    // Ideally we would store the flux as a symmetric tensor instead, but this
    // requires changes to how the flux tags are constructed.
    auto longitudinal_shift =
        make_with_value<tnsr::II<DataVector, 3>>(shift_strain, 0.);
    Xcts::euclidean_longitudinal_shift(make_not_null(&longitudinal_shift),
                                       shift_strain);
    for (size_t i = 0; i < 3; ++i) {
      for (size_t j = 0; j < 3; ++j) {
        flux_for_shift->get(i, j) = longitudinal_shift.get(i, j);
      }
    }
  }
  static void apply(
...
struct Fluxes<Equations::HamiltonianLapseAndShift, Geometry::NonEuclidean> {
  ...
  static void apply(
    ...
    auto longitudinal_shift =
        make_with_value<tnsr::II<DataVector, 3>>(shift_strain, 0.);
    Xcts::longitudinal_shift(make_not_null(&longitudinal_shift),
                             inv_conformal_metric, shift_strain);
    for (size_t i = 0; i < 3; ++i) {
      for (size_t j = 0; j < 3; ++j) {
        flux_for_shift->get(i, j) = longitudinal_shift.get(i, j);
      }
 ...
 *
 * Adds \f$-\frac{1}{32}\frac{\psi^{5}}{\alpha^2} \left((\bar{L}\beta)^{ij}
 * -\bar{u}^{ij}\right) \left((\bar{L}\beta)_{ij}-\bar{u}_{ij}\right)\f$.
 *
 * \see `Xcts::FirstOrderSystem`
 */
 ...
 * source to the Hamiltonian constraint or lapse equation
 *
 ...
 * background and with \f$\bar{u}_{ij}=0=\beta^if$.
 *
 ...
 * lapse equation.
 *
 * Adds \f$-\frac{1}{32}\frac{\psi^{7}}{(\alpha\psi)^2}
 * \left((\bar{L}\beta)^{ij} - \bar{u}^{ij}\right)
 * \left((\bar{L}\beta)_{ij} - \bar{u}_{ij}\right)\f$ to the Hamiltonian
 * constraint and \f$\frac{7}{32}\frac{\psi^{6}}{\alpha\psi}
 * \left((\bar{L}\beta)^{ij} - \bar{u}^{ij}\right)
 * \left((\bar{L}\beta)_{ij} - \bar{u}_{ij}\right)\f$ to the lapse equation.
 *
 * \see `Xcts::FirstOrderSystem`
 */
 ...
 *
 * Also adds \f$-\frac{1}{32}\frac{\psi^7}{\left(\alpha\psi\right)^2}
 * \left((\bar{L}\beta)^{ij} - \bar{u}^{ij}\right)
 * \left((\bar{L}\beta)_{ij} - \bar{u}_{ij}\right)\f$ to the Hamiltonian
 * constraint and \f$\frac{7}{8}\frac{\psi^6}{\alpha\psi}
 * \left((\bar{L}\beta)^{ij} - \bar{u}^{ij}\right)
 * \left((\bar{L}\beta)_{ij} - \bar{u}_{ij}\right)\f$ to
 * the lapse equation.
 *
 * \see `Xcts::FirstOrderSystem`
 */
