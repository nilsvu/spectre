...
void longitudinal_shift(
  ...
  static constexpr bool is_symmetric = std::is_same_v<Symm, Symmetry<1, 1>>;
  static_assert(
      is_symmetric or std::is_same_v<Symm, Symmetry<1, 2>>,
      "This function is implemented only for symmetric result tensors or "
      "tensors with no symmetry.");
    ...
    for (size_t j = 0; j <= i; ++j) {
      for (size_t k = 0; k < 3; ++k) {
        for (size_t l = 0; l < 3; ++l) {
          ...
          result->get(i, j) += projection * shift_strain.get(k, l);
        }
      }
    }
    // Fill remaining components in non-symmetric result tensor
    if constexpr (not is_symmetric) {
      for (size_t j = i + 1; j < 3; ++j) {
        result->get(j, i) = result->get(i, j);
      }
    }
  }
}
...
void euclidean_longitudinal_shift(
  ...
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j <= i; ++j) {
      result->get(i, j) = 2. * shift_strain.get(i, j);
    }
    result->get(i, i) -= shift_strain_trace_term;
    // Fill remaining components in non-symmetric result tensor
    if constexpr (not is_symmetric) {
      for (size_t j = i + 1; j < 3; ++j) {
        result->get(j, i) = result->get(i, j);
...
#define INSTANTIATE_DTYPE(_, data)                            \
  ...
  template void longitudinal_shift(                           \
      const gsl::not_null<tnsr::IJ<DTYPE(data), 3>*> result,  \
      const tnsr::II<DTYPE(data), 3>& inv_conformal_metric,   \
      const tnsr::ii<DTYPE(data), 3>& shift_strain) noexcept; \
