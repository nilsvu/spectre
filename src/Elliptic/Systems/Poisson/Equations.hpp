...
/*!
 * \brief Add the sources \f$S=-\Gamma_{ijk}\gamma^{ij}v^k\f$
 * for the curved-space Poisson equation on a spatial metric \f$\gamma_{ij}\f$.
 *
 ...
 */
template <size_t Dim>
struct Fluxes<Dim, Geometry::Euclidean> {
 ...
 */
template <size_t Dim>
struct Fluxes<Dim, Geometry::NonEuclidean> {
  using argument_tags = tmpl::list<
  ...
  static void apply(
      ...
      const tnsr::II<DataVector, Dim>& inv_spatial_metric,
      const tnsr::i<DataVector, Dim>& field_gradient) noexcept {
    non_euclidean_fluxes(flux_for_field, inv_spatial_metric, field_gradient);
  }
  static void apply(
      const gsl::not_null<tnsr::Ij<DataVector, Dim>*> flux_for_gradient,
      const tnsr::II<DataVector, Dim>& /*inv_spatial_metric*/,
 ...
 */
template <size_t Dim>
struct Sources<Dim, Geometry::Euclidean> {
  using argument_tags = tmpl::list<>;
  static void apply(
 ...
 */
template <size_t Dim>
struct Sources<Dim, Geometry::NonEuclidean> {
  using argument_tags =
  ...
  static void apply(
      ...
      const tnsr::I<DataVector, Dim>& field_flux) noexcept {
    get(*source_for_field) = 0.;
    add_non_euclidean_sources(source_for_field, christoffel_contracted,
                              field_flux);
  }
};
