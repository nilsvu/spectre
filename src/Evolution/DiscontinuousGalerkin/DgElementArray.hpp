 ...
 *
 * Requires that the `InitialData` exposes an `import_fields` typelist of tags.
 * These will be imported in the `ImportPhase` from the data file that is
 * specified by the options in the `evolution::OptionTags::NumericInitialData`
 * group.
 *
 * \see `DgElementArray`
 */
...
struct try_import_data<
  ...
  static void apply(
    ...
    static_assert(
        cpp17::is_same_v<PhaseType, typename Metavariables::Phase>,
