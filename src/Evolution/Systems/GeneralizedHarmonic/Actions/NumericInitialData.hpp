 ...
 * - ::Tags::Time: To compute numerical derivatives on time-dependent domains we
 *   have to temporarily set the time to the initial time, so it gets picked up
 *   by compute items for the Jacobian. We set the time back to NaN before this
 *   action completes.
 *
 * \tparam ImporterOptionsGroup Option group in which options are placed.
 */
...
struct SetNumericInitialData {
  ...
  apply(db::DataBox<DbTagsList>& box,
    ...
    if (std::holds_alternative<detail::GeneralizedHarmonic>(
      ...
      // To compute the Jacobian on time-dependent domains we have to set a
      // time, which gets picked up compute tags and passed to the functions of
      // time. During initialization the time is NaN, so here we set it to the
      // InitialTime and then back to NaN.
      const auto& domain = db::get<domain::Tags::Domain<Dim>>(box);
      const auto& block = domain.blocks()[element_id.block_id()];
      const bool is_time_dependent = block.is_time_dependent();
